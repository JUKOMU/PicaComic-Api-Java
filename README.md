...
* **网络处理**:
    ...
    * **图片来源**: 图片 URL 由模型或调用方提供并交给 OkHttp 解析；图片 client 不带 API 凭据，使用默认 redirect，接受成功的 `2xx` body 和标准 gzip。
* **并发下载**:
    ...
    * 下载文件先完整写入同目录 `.part` 文件，再尝试 `ATOMIC_MOVE`；文件系统不支持时使用 `REPLACE_EXISTING`，失败或取消都会清理临时文件。
...
`downloadThreadPoolSize` 默认值为 12，`concurrentPhotoDownloads` 默认值为 3，`concurrentImageDownloads` 默认值为 20；三者都必须为正数，且没有库级硬上限。`imageTimeout` 默认值为 60 秒，只用于图片读取。使用 `loadFromProperties` 时并发键为 `download.thread.pool.size`、`concurrent.photo.downloads` 和 `concurrent.image.downloads`，图片超时键为 `image.timeout.seconds`。
